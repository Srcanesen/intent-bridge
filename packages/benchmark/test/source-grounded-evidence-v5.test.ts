...
const v5 = join(root, "benchmarks", "source-grounded-evidence-v5");
...
const pass = () => ({
  schemaVersion: 1,
  benchmarkId: "source-grounded-evidence-v5",
  subjectRelease: "v1.2.0-rc",
  subjectCommit: "74f8856d0c903562a4a40e9240f8dd92b04b6b56",
  manifestSha256: SOURCE_GROUNDED_EVIDENCE_V5_MANIFEST_SHA256,
...
describe("source-grounded-evidence-v5 frozen preregistration", () => {
  it("accepts the exact manifest, exact file hash, and referenced v1 corpus", async () => {
    const bytes = await readFile(join(v5, "manifest.json"));
    ...
    expect(
      validateSourceGroundedEvidenceV5Corpus(cases, annotations).cases,
    ).toHaveLength(8);
  ...
  it("rejects manifest identity, prompt, cap, gates, review, and unknown-key drift", async () => {
    const manifest = await json(join(v5, "manifest.json"));
    for (const changed of [
      { ...manifest, benchmarkId: "source-grounded-evidence-v4" },
      ...
      expect(() => parseSourceGroundedEvidenceManifestV5(changed)).toThrow();
  });

  it("accepts stopped smoke and approved pass aggregates", () => {
    const valid = pass();
    expect(parseSourceGroundedEvidenceAggregateResultV5(valid).decision).toBe(
      "pass",
    );
    const stopped = {
      ...valid,
    ...
    expect(parseSourceGroundedEvidenceAggregateResultV5(stopped).decision).toBe(
      "stop",
    );
  });
  ...
  it("rejects identity, manifest, raw, cap, gate, review, and approval drift", () => {
    ...
    for (const changed of [
      ...
      expect(() =>
        parseSourceGroundedEvidenceAggregateResultV5(changed),
      ).toThrow();
  });
});

describe("v4 regression passes unchanged", () => {
  it("accepts v4 exact manifest and referenced corpus", async () => {
    const { parseSourceGroundedEvidenceManifestV4 } = await import(
      "../src/index.js"
    );
    const bytes = await readFile(
      join(root, "benchmarks", "source-grounded-evidence-v4", "manifest.json"),
    );
    expect(() =>
      parseSourceGroundedEvidenceManifestV4(JSON.parse(bytes.toString())),
    ).not.toThrow();
  });
});

describe("v3 regression passes unchanged", () => {
...
describe("v2 regression passes unchanged", () => {
...
describe("v1 regression passes unchanged", () => {
