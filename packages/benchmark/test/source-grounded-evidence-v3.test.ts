...
const v3 = join(root, "benchmarks", "source-grounded-evidence-v3");
...
describe("source-grounded-evidence-v3 frozen preregistration", () => {
  it("accepts the exact manifest, exact file hash, and referenced v1 corpus", async () => {
    ...
    expect(
      validateSourceGroundedEvidenceV3Corpus(cases, annotations).cases,
    ).toHaveLength(8);
  ...
  it("rejects manifest identity, prompt, cap, gates, review, and unknown-key drift", async () => {
    const manifest = await json(join(v3, "manifest.json"));
    for (const changed of [
      { ...manifest, benchmarkId: "source-grounded-evidence-v2" },
      ...
      expect(() => parseSourceGroundedEvidenceManifestV3(changed)).toThrow();
  });

  it("accepts stopped smoke and approved pass aggregates", () => {
    const valid = pass();
    expect(parseSourceGroundedEvidenceAggregateResultV3(valid).decision).toBe(
      "pass",
    );
    const stopped = {
      ...valid,
    ...
    expect(parseSourceGroundedEvidenceAggregateResultV3(stopped).decision).toBe(
      "stop",
    );
  });
  ...
  it("rejects identity, manifest, raw, cap, gate, review, and approval drift", () => {
    ...
    for (const changed of [
      ...
      expect(() =>
        parseSourceGroundedEvidenceAggregateResultV3(changed),
      ).toThrow();
  });
});
