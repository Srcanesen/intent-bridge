# Kaynağa dayalı kanıt v2

Bu belge, Pi-native v5 doğrudan şema düzeltmesi için `source-grounded-evidence-v2` dondurulmuş ön kaydıdır. Konu sürümü `v1.2.0-rc`, konu taahhüdü `acb8f60e5f2a0e6297c0f1bc01853b0b5ee12294` ve Pi promptu `pi-native-v5`tir.
...
V1 stopped sonucu tarihsel ve bağımsızdır; bu ön kayıt onu değiştirmez veya yeniden yorumlamaz. PR/CI yalnızca çevrimdışı doğrulama yapar, canlı çağrı yapmaz.
...
```bash
corepack pnpm benchmark -- sge-v2 validate-manifest
corepack pnpm benchmark -- sge-v2 validate-aggregate /yerel/sanitize-edilmis-sonuc.json
